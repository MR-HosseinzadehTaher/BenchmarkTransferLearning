...
from trainer import train_one_epoch_classification,train_one_epoch_segmentation, test_classification,evaluate_classification,evaluate_segmentation,test_segmentation
from utils import learning_rate_schedule,dice,mean_dice_coef
  ...
  if args.mode == "train":
    ...
    for i in range(args.start_index, args.num_trial):
      ...
      if args.optimizer.lower() == "adam":
        ...
        optimizer = torch.optim.SGD(parameters, lr=args.lr, weight_decay=args.weight_decay, momentum=args.momentum, nesterov=False)
      ...
      for epoch in range(start_epoch, args.num_epoch):
        train_one_epoch_classification(data_loader_train, device, model, criterion, optimizer, epoch, apply_activation=False)
        val_loss = evaluate_classification(data_loader_val, device, model, criterion, apply_activation=False)
...
def segmentation_engine(args, model_path, dataset_train, dataset_val, dataset_test,criterion):
  ...
  model, optimizer,loss_scaler = build_segmentation_model(args, log_writter)
  if args.mode == "train":
    start_num_epochs=0
    data_loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True,
                                                 num_workers=args.train_num_workers, drop_last=False, pin_memory=True)
    data_loader_val = torch.utils.data.DataLoader(dataset_val, batch_size=args.train_batch_size,
                                                      shuffle=False, num_workers=args.train_num_workers,drop_last=False, pin_memory=True)
    best_val_loss = 100000
    patience_counter = 0
    for epoch in range(start_num_epochs, args.epochs):
        lr_ = learning_rate_schedule(epoch, args.epochs, args.learning_rate)
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr_
        print('learning_rate: {},{}'.format(optimizer.param_groups[0]['lr'], epoch), file=log_writter)
        loss_avg = train_one_epoch_segmentation(model,data_loader_train, optimizer, loss_scaler, epoch,criterion,log_writter)
        print('Training loss: {}@Epoch: {}'.format(loss_avg, epoch), file=log_writter)
        log_writter.flush()
        val_avg = evaluate_segmentation(model,data_loader_val,epoch,criterion,log_writter)
        if val_avg < best_val_loss:
            ...
            print( "Epoch {:04d}: val_loss improved from {:.5f} to {:.5f}, saving model to {}".format(epoch, best_val_loss,val_avg, os.path.join(model_path, "checkpoint.pt")), file=log_writter)
            best_val_loss = val_avg
            patience_counter = 0
        else:
            print("Epoch {:04d}: val_loss did not improve from {:.5f} ".format(epoch, best_val_loss), file=log_writter)
            patience_counter += 1
        if patience_counter > args.patience:
            print("Early Stopping", file=log_writter)
            break

        log_writter.flush()
  torch.cuda.empty_cache()
  data_loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.test_batch_size, shuffle=False,
                                                num_workers=args.test_num_workers,drop_last=False, pin_memory=True)
  test_y, test_p = test_segmentation(args, os.path.join(model_path, "checkpoint.pt"), data_loader_test, log_writter)
